...
func TestBinlogEventNextPosition(t *testing.T) {
	input := binlogEvent(googleQueryEvent)
	want := int64(0x049a)
	if got := input.NextPosition(); got != want {
		t.Errorf("%#v.ServerID() = %v, want %v", input, got, want)
	}
}
...
func TestBinlogEventRotate(t *testing.T) {
	...
	input := binlogEvent(googleRotateEvent)
	wantFilename := "62344-bin.000001"
	wantOffset := int64(0x30303030302d7476)
	if filename, offset, err := input.Rotate(f); wantFilename != filename || wantOffset != offset || err != nil {
		t.Errorf("TestBinlogEventRotate (filename:%v offset:%v error:%v) != want(filename:%v offset:%v error:nil) ", filename, offset, err, wantFilename, wantOffset)
	}
...
func TestBinlogEventQuery(t *testing.T) {
	...
	want := Query{
		...
		Charset:  &Charset{Client: 8, Conn: 8, Server: 33},
