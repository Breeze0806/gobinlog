...
// parseGTIDSet calls the parser for the specified flavor.
func parseGTIDSet(flavor, value string) (GTIDSet, error) {
	parser := gtidSetParsers[flavor]
	if parser == nil {
		return nil, fmt.Errorf("parse error: unknown GTIDSet flavor %#v", flavor)
	}
...
func TestMysql56ParsePosition(t *testing.T) {
	...
	got, err := parseGTIDSet(mysql56FlavorID, input)
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
	if !got.Equal(want) {
		t.Errorf("(&mysql56{}).parseGTIDSet(%#v) = %#v, want %#v", input, got, want)
	}
}
